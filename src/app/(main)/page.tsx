  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0f172a] via-[#1a1a2e] to-[#16213e]">
      <main className="relative z-10 max-w-7xl mx-auto px-6 py-10 pb-20">
        {/* Secciones */}
        {enEuskera.length > 0 && <CarouselSection title="Euskaraz ikusgai" items={enEuskera} />}
        {tendencia.length > 0 && <CarouselSection title="Tendencia orain" items={tendencia} icon="🔥" />}
        {nuevas.length > 0 && <CarouselSection title="Berri-berriak" items={nuevas} icon="🆕" />}
        {netflix.length > 0 && <CarouselSection title="Netflix-en euskaraz" items={netflix} icon="N" />}
        {etb.length > 0 && <CarouselSection title="ETB-n ikusgai" items={etb} icon="📺" />}
      </main>
    </div>
  );
}
